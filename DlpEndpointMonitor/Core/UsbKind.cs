...
static class DeviceKindResolver
{
    ...
    static readonly Dictionary<string, int> _guidToClass = new(StringComparer.OrdinalIgnoreCase)
    {
        ...
        // ── Video ─────────────────────────────────────────────────────────────
        // NOTE: KSCATEGORY_CAPTURE {65E8773D} is deliberately NOT mapped to video. It is
        // an AUDIO+VIDEO capture category - HD-audio codecs register their mic/speaker/
        // HDMI-audio topology under it (wavemicin, wavespeaker, ehdmiouttopo), so mapping
        // it to 0x0E made a "video" policy sweep up onboard audio endpoints and try to
        // disable them. KSCATEGORY_VIDEO {6994AD05} is video-specific and is what real
        // UVC webcams register under, so it alone is the reliable video signal.
