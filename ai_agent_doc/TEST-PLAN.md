...
| # | Setup | Action | Expected |
|---|---|---|---|
| M-ALERT-01 (visual shape, both types) | None | Call `ShowAlert` once for each of `Toast`/`FullScreen`, each severity | Each shows the same rounded 20px-corner-radius box with a colored header band (Info blue `#1B7FA9`, Warning amber `#EB980A`, Blocked red `#EE3A3A`) and a white body; `FullScreen` additionally fills the entire screen edge-to-edge with the severity color as backdrop |
